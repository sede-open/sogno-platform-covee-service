...
load_dotenv()
control_variables = os.getenv('CONTROL_VARIABLES')
ext_port = os.getenv('EXT_PORT_POWERFLOW')
...
try:
    while True:
        ...
        measurements={"VMAX":1.05}
        measurements.update({"VMIN":18e3})
        measurements.update({"voltage_measurements": [v_tot[int(full_nodes[i])-1] for i in range(len(full_nodes))]})
        measurements.update({"pv_input_measurements": [ p[i] for i in range(len(p))]})
        measurements.update({"active_power_control_dict": [p_value[i] for i in range(len(p_value))]})
        measurements.update({"reactive_power_control_dict": [q_value[i] for i in range(len(q_value))]})
        measurements.update({"active_power_ESS_control_dict": [p_ESS_value[i] for i in range(len(p_ESS_value))]})      
        measurements.update({"reactive_percentage": [(-q_value[i]-0.31512) for i in range(len(q_value))]})      
    ...
    rows = voltage_tot
    with open('covee-powerflow/csv_files/voltage.csv', 'w+', encoding="ISO-8859-1", newline='') as csv_file:
        wr = csv.writer(csv_file)
        for row in rows:
            wr.writerow(row)
    csv_file.close()
    ...
    rows = active_power_pv_tot
    with open('covee-powerflow/csv_files/active_power_pv_tot.csv', 'w+', encoding="ISO-8859-1", newline='') as csv_file:
        wr = csv.writer(csv_file)
        for row in rows:
            wr.writerow(row)
    csv_file.close()
    ...
    rows = reactive_power_pv_tot
    with open('covee-powerflow/csv_files/reactive_power_pv_tot.csv', 'w+', encoding="ISO-8859-1", newline='') as csv_file:
        wr = csv.writer(csv_file)
        for row in rows:
            wr.writerow(row)
    csv_file.close()
    ...
    rows = active_power_ess_tot
    with open('covee-powerflow/csv_files/active_power_ess_tot.csv', 'w+', encoding="ISO-8859-1", newline='') as csv_file:
        wr = csv.writer(csv_file)
        for row in rows:
            wr.writerow(row)
    csv_file.close()
    ...
    rows = load_tot
    with open('covee-powerflow/csv_files/load_tot.csv', 'w+', encoding="ISO-8859-1", newline='') as csv_file:
        wr = csv.writer(csv_file)
        for row in rows:
            wr.writerow(row)
    csv_file.close()
    ...
    rows = active_nodes_list
    with open('covee-powerflow/csv_files/active_nodes_list.csv', 'w+', encoding="ISO-8859-1", newline='') as csv_file:
        wr = csv.writer(csv_file)
        for row in rows:
            wr.writerow(row)
    csv_file.close()
