...
class read_profiles:
    ...
    def read_csv(self):
        ...
        wd.replace("\\", "/")
        with open(os.path.join(wd, '10_nodes_PV.csv')) as csv_file: #10 Nodes: 10_nodes_PV # No Cloud: PV_profile_LV_SOGNO_4kw  #With Cloud: PV_profile_LV_SOGNO-cloudlevel-2.csv
            pvproduction = csv.reader(csv_file, delimiter=',')
            x = list(pvproduction)
            self.pvproduction = np.array(x).astype("float")

        with open(os.path.join(wd, '10_nodes_Load.csv')) as csv_file:  # 10 Nodes: 10_nodes_Load    # LOAD_profile_LV_SOGNO
